...
Date: 2026-09-22. Every İAA differentiator claim is tested as a falsifiable hypothesis
...
(not policies); BMAD = product-development methodology with named personas; İAA =
delegation-decision policy layer.

---

## H1 — "İAA uniquely decides whether delegation is worthwhile before deciding how to delegate"

**İAA evidence:** SKILL.md benefit test (5 benefits vs coordination cost; C17/C18 PROVEN —
...
with cost guidance. Still differentiated: (a) İAA's benefit gate is the *core* of one
...
as policy; (c) İAA holds **behavioral evidence** that the gate works (campaigns: Task-1
...
## H2 — "İAA justifies each agent seat independently"

**İAA evidence:** SKILL.md:21 — every implementer/reviewer/re-reviewer/fixer seat requires
...
seats. Caveat: İAA's own evidence records legitimate reviewer-count variance (0 vs 2) —
the rule is a judgment policy, not a determinizer, by design.

## H3 — "İAA supports mixed primary + delegated execution without requiring a complete workflow transition"

**İAA evidence:** C23 PROVEN — primary keeps coupled/shared-contract work while workers
...
| Codex | Verified in İAA's own native audit (root worked concurrently with children; ADR-0000). Again mechanism, not protocol. | ADR-0000 [PROVEN, local] |
...
İAA couples mixed execution to an explicit write-ownership protocol (exclusive write sets,
...
declared scope", 1.11/1.13) — mechanistically *stronger* than İAA's brief-based prose,
though scoped to GSD-managed repos only.

## H4 — "İAA treats explicit orchestration ownership and artifact trust boundaries as first-class concepts"

**İAA evidence:** two-mode mutual exclusion selected at skill-load time (C2/C3 PROVEN);
...
|---|---|---|
| Superpowers 6.4.1 | None (no awareness of İAA or other controllers; no per-skill disable). | Opposite direction: writing-plans *embeds* "REQUIRED SUB-SKILL" directives in every plan (verbatim unchanged in 6.4.1) — it is the artifact-injection source İAA defends against. | 
...
advisory scanner, for different purposes. Honest bounds: İAA's boundary is prose-enforced
...
## H5 — "İAA is methodology-agnostic"

**İAA evidence:** core policy is task-shape-based (no phases/artifacts mandated); adapter
docs are mechanism-mappings only.

Tests against İAA itself (the hypothesis cuts inward, too):
...
2. The **boundary layer is Superpowers-shaped**: SKILL.md names `superpowers:
   ...
   Superpowers-specific; ALL behavioral evidence is İAA-vs-SDD. The generic wording ("any
...
3. Under a foreign methodology (GSD-managed repo, BMAD project), İAA's behavior is
...
## H6 — "İAA is lightweight because it is pure policy rather than a framework"
...
no observability surface (no ledger — cannot post-hoc answer "what did İAA decide" without
transcript analysis; Superpowers ships diagnosing-superpowers for this, GSD /gsd-forensics,
İAA nothing); no cross-session resume (SDD ledger exists because "controllers that lost
their place have re-dispatched entire completed task sequences — the single most expensive
failure observed" — a failure class İAA does not address); upgrade fragility (one-way
...
17 install targets; BMAD ~47 platforms. İAA covers 3. İAA's distinct nuance: the
...
the ecosystems themselves are converging on cross-compat, eroding the uniqueness of İAA's
hand-rolled version of it.

## H8 — "Anti-overdelegation is İAA's headline objective" (added)
...
comparable single-agent runs"; Claude teams: token cost "Higher") without gating. İAA's
$2.90–3.27 (İAA mode) vs $9.57 (co-loaded) vs $8.16–11.24 (native SDD) measurements on a
...
flags; scope validation) — mechanistically stronger than İAA's prose "dependency-aware
waves." ZCode 3.14/Claude native dynamic workflows make topology an explicit script
artifact. What remains İAA-specific: topology adaptation *down to zero agents* within a
...
**CONFIRMED DIFFERENTIATOR (with a Superpowers caveat).** İAA answers "use subagents where
...
|---|---|---|
| Adaptive topology selection (H1/H9/H10) | fewer agents / lower cost / less context pressure | consequences; cost advantage evidenced ONLY within İAA's own seed/model campaign |
...
1. Baseline research/01 carried "user instructions take precedence over skills" as an
   ...
   Superpowers' own bootstrap concedes it). İAA's C8 remains PROVEN empirically + by
...
2. ADR-0000's fork_turns "(none/all/suffix)" vs raw diagnostics (none/all only) — Erratum
   E-2 (00 §5).
3. BMAD v6.5.0 "42 platforms" vs 47 yaml entries today — immaterial to İAA; noted in
   stream file only.
...
only; none made comparative judgments, modified files, invoked İAA/any framework, or
