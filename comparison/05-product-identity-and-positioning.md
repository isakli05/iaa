...
Date: 2026-09-22. What category MAO actually occupies, chosen from observed architecture
...
| Candidate category | Fits MAO? | Evidence/counter-evidence |
|---|---|---|
| Multi-agent *skill* | Partly — form factor yes, but "multi-agent skill" suggests it spawns/coordinates agents itself; MAO decides *whether others* should | MAO creates no agents (C36); it is consumed by the primary agent |
| Orchestration *framework* | No | Frameworks in the wild (GSD, BMAD) ship state, artifacts, roles, installers that transform the environment; MAO ships none (C36, C11) |
| *Workflow engine* | No | Engines are deterministic code (Claude/ZCode dynamic workflows); MAO has no scripts at execution time |
| *Methodology* | No | Methodologies own phases/artifacts (BMAD "Clarify→Plan→Build"; GSD five-phase loop); MAO is task-shape-driven, artifact-free |
...
| Runtime *adapter* | No — that's a component | Adapters are MAO's reference docs, not the product |

**Occupied category (chosen):** MAO is a **delegation-decision and orchestration-arbitration
...
MAO is the missing *policy layer* that decides when those mechanisms should be used at all
and which workflow authority governs a task.

## 2. What the category choice must honestly exclude

- Not "orchestration framework" (would claim state/roles/artifacts MAO doesn't have).
- Not "agent coordinator" (MAO never coordinates agents directly; the primary agent does,
  under MAO's policy).
...
- Positioning *against* SDD is a category error to avoid publicly: SDD is a fixed-cadence
  execution workflow (its own design goal: review-gated quality); MAO is a policy for
...
MAO is a runtime-agnostic, instruction-channel policy skill for coding-agent CLIs. When a
...
MAO is a small rulebook you install that teaches your AI coding assistant *when using
...
you explicitly name another workflow (like Superpowers SDD), MAO steps aside completely
...
> **Multi-Agent Orchestration (MAO)** is a portable policy skill for Claude Code, Codex
> CLI, and ZCode that governs how the primary agent decides *whether, when, and how* to
> delegate work to subagents. Instead of a fixed workflow, MAO applies a concrete benefit
...
> MAO by default, or any named workflow you explicitly request (MAO then stands down);
> workflow directives embedded in plans or repository files never switch controllers. MAO
...
1. **Convergence pressure on the core idea:** vendor policy templates now encode
   ...
   docs heuristics) — MAO must not be positioned as "the idea of sensible delegation" but
...
2. **The arbitration layer is valuable precisely because nobody else has it** (H4) — but
   it is also unreciprocated; positioning must promise "MAO never loads a competitor and
   yields on explicit request," never "other frameworks yield to MAO."
