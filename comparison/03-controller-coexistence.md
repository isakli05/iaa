...
| Codex native | None | n/a | name collisions "both can appear in skill selectors" (not merged) | none found | `allow_implicit_invocation` per skill (default true); Ultra-tier **proactive delegation template** ("materially improve speed or quality") — a prose policy that would share instruction space with İAA's AGENTS.md shim if both active (untested) | same |
...
**Structural result:** *no system reciprocates anyone else's authority.* İAA's mutual
exclusion is one-sided by necessity — it works because İAA declines to load competitors
...
### İAA + Superpowers (installed together)
...
verbatim-unchanged (00 §4B). Classes: I=OK (plugin-scoped cache; İAA touches nothing
foreign, mtime-swept), N=OK (namespaced), T=**permanently open flank** (both
`subagent-driven-development` and `dispatching-parallel-agents` descriptions overlap İAA's
trigger surface; no per-skill disable exists upstream), O=resolved-by-İAA (exclusion;
...
### İAA + GSD (not installed locally — analysis from primary sources; untested behaviorally)
Baseline matrix scored this pair **unknown across I/T/O** with the specific worry "GSD's
PreToolUse workflow guards may gate İAA's own dispatches." Current evidence **resolves the
...
- **O/T:** GSD's agent-isolation guard hard-blocks only `gsd-executor` dispatches in
  ...
  dispatch "returns allow" before GSD state is even consulted** (stream-gsd §7). İAA's
  Explore/general-purpose dispatches inside a GSD-managed repo would pass untouched. The
  reverse direction (GSD global skills' broad descriptions competing with İAA's
...
- **I:** GSD's npm installer writes 23 JS + 5 shell hook files into `~/.claude/hooks/` and
  managed entries in settings; İAA's artifacts are disjoint paths (symlink, CLAUDE.md
  ...
  bootstrap and İAA's shim (all coexist as separate hook entries — Claude hooks "merge
  across settings levels").
- **Open question (P4 gap, unchanged):** İAA's by-name yield rule ("a native workflow…
...
ask for one" → if a user enables teams, İAA-authorized dispatches could silently become
teammates (full sessions, higher token cost, no worktree isolation — "partition the
work"), changing topology semantics under İAA's feet; (b) no project-level team config
exists, so İAA cannot detect or defer to a team declaratively. Also: SendMessage/ListAgents
give *peer* identity that cuts across İAA's root-to-child model. Untested; flag remains a
real interaction class, now with a concrete mechanism.

### İAA + native Codex multi-agent
...
allows unbounded child-spawns-child nesting — İAA's Codex nesting stance (policy-only)
...
delegation-trigger policies (template + İAA shim) share one instruction space — same
instruction channel, compatible *values* (both gate on material benefit) but untested
interaction.

### İAA + ZCode plugin/subagent mechanisms
İAA's ZCode integration rides documented public surfaces (skills symlink, AGENTS.md shim)
...
are user-invoked script orchestration — opt-in like Claude workflows; collision with İAA
only if invoked inside an İAA-governed task (untested, same class as Claude Workflow).

### İAA + BMAD (not installed)
No coexistence design on BMAD's side; BMAD's surface is per-project skill dirs + managed
repo-AGENTS.md block + named agents. Overlap with İAA: bmad-build's own delegation
(reviewer subagents) would run *inside* whatever controller governs the session — if İAA's
shim is active, İAA's seat-materiality policy and BMAD's mandatory-review stage prescribe
...
foreign methodology a public İAA user will have installed.

### İAA + Claude native dynamic workflows (added)
...
only by explicit user invocation inside an İAA task — a P4-style by-name case İAA's yield
rule nominally covers, untested (07-E10).

## 3. Cross-cutting findings

1. **Nobody enforces authority; two systems enforce invariants; one system (İAA) declares
   authority.** Coexistence today = (İAA's one-sided exclusion) + (everyone's mutual
...
2. **The universal collision channel is the trigger listing**, not files: every
   model-invocable broad-description skill (İAA, SDD, dispatching-parallel-agents, GSD's
...
3. **SessionStart injection is an arms race İAA declined to enter** — and the only system
...
4. **This comparison session is itself a live coexistence datum** (2026-09-22, recorded):
   ...
   session), İAA shim present via CLAUDE.md, user task prompt explicitly forbidding
...
No. Inventory across all researched systems: ownership protocols — **none** (İAA only);
explicit yield rules — **none** (İAA only, SDD-tested); namespace only — Superpowers
...
unoccupied outside İAA** — the strongest single differentiator finding of this comparison
