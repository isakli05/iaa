...
expected_outcome: Scenario J. SDD is never invoked; the plan's embedded "REQUIRED SUB-SKILL" directive is treated as metadata; İAA governs with adaptive topology. DORMANT — requires Superpowers to be co-loaded, which plugin-eval containment prevents (see evals/README.md); run the behavioral companion test instead.
