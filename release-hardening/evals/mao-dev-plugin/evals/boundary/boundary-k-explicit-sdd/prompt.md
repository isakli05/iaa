...
description: Explicit native SDD opt-in — the named workflow governs and MAO yields
expected_outcome: Scenario K. An explicit by-name request loads superpowers:subagent-driven-development and runs its own cadence; the multi-agent-orchestration skill is NOT loaded as a competing authority. DORMANT — requires Superpowers to be co-loaded, which plugin-eval containment prevents (see evals/README.md); run the behavioral companion test instead.
