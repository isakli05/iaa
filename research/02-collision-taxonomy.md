...
- **İAA today:** installs only its own canonical dir + 3 self-owned symlink paths + 3
...
- **Residual local Class-1 facts:** graphify and İAA both edit the *same* global instruction
  files (different sections, both preserved — proven coexistence); ZCode's stale Superpowers
  6.2.0 cache with a broken /tmp symlink is an upstream/plugin-manager artifact, not an İAA
  collision.
- **Risk profile for public distribution:** LOW by construction, but any future packaging
  change (plugin install vs raw skills dir) reopens it (two İAA copies installed twice =
...
- **İAA today:** one un-namespaced user-scope skill name `iaa`; no
  commands; no agents. No other installed system claims that name (verified). Superpowers
  skills are plugin-namespaced (`superpowers:*`), so SDD vs İAA is *not* a Class-2 collision
  locally.
- **Public risk:** REAL — any other user's plugin/skill named `iaa`
  (or a second İAA installed at both personal scope and plugin scope) creates shadowing.
...
- **Local instance:** İAA ("use subagents, delegate, divide, parallelize…") vs Superpowers
  ...
  moved the contest from *in-context precedence* to *selection-time routing* (İAA's
...
- **Structural asymmetry:** Superpowers' SessionStart hook injects a must-invoke-skills
  bootstrap every session; İAA has no hook. İAA's counter is riding the user-instruction
...
- **Public risk:** HIGH — this is the class that actually produced the historical failure,
  and any new orchestration plugin with a broad proactive description re-creates it. İAA's
...
- This is the class campaigns 1–3 lived in: İAA says "adaptive, 3 implementers, 0-2
...
- **Public risk:** INHERENT — any two authoritative workflow systems collide in this class
  ...
  authority. These are exactly İAA's three current rules (SKILL.md:12–21); the open question
  for public release is whether *other* systems reciprocate (SDD does not know İAA exists).

## Non-collision interactions (for completeness)

- **Mechanism vs policy:** Codex MultiAgentV2 / Claude subagents are mechanisms İAA *uses* —
...
- **Retrieval layers:** graphify overlaps triggers with everything codebase-related but
  claims no execution authority; İAA explicitly defers to project orientation policies first
  (SKILL.md:33).
