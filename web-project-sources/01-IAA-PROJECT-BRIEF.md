...
2. **Sole orchestration authority per task, by mode.** Adaptive İAA mode (default) never
   loads a competing orchestration engine (specifically Superpowers SDD); a native workflow
   runs only on explicit by-name user request, and then İAA stands down entirely. The two
...
is genuinely differentiated — using this pack as the accurate description of İAA. No
