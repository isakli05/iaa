...
1. **One authority per task.** Adaptive İAA (default) ⟷ native workflow (explicit by-name
   opt-in only). Mutually exclusive; never composed.
2. **İAA mode never loads SDD.** Not "overrides" — never loads. (Prose precedence was tried
...
5. **Component skills stay usable in İAA mode** (whitelist: TDD, git-worktrees,
...
6. **In native mode İAA stands down entirely** (verified: full SDD cadence ran — 15 and 11
   agents in two tests — with İAA never loaded).
7. **Neither side modified.** No Superpowers file/setting/hook touched, ever (mtime-swept).

## What each engine claims (contrast table)

| | İAA | SDD 6.3.0 (as installed) |
|---|---|---|
...
İAA-mode runs: $2.90–$3.27, 3 parallel implementers, 0 reviewers, 38–42 tests green.
