...
| Invocation | description-matched + `/multi-agent-orchestration` | implicit + `$multi-agent-orchestration` | per-turn description injection (≤250 chars) + `$multi-agent-orchestration` |
...
- Codex user skills dir **is** `~/.agents/skills` (early docs said `~/.codex/skills`; that
  dir now holds only Codex-managed system skills) — MAO's symlink is correctly placed.
- ZCode: description hard limit 1024 chars, per-turn injection ~250 chars (MAO's
  description is 245–249 chars by design); subagents cannot spawn subagents (matches MAO's
  adapter); since ZCode 3.7.1 subagents inject AGENTS.md (except built-in Explore) — matches
  MAO's adapter text written at install time against 3.7.7.
- Claude Code: non-fork subagents inherit CLAUDE.md (the shim reaches workers); built-in
  ...
  locally capped to 1 by MAO's managed env key.
