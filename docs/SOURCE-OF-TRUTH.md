...
- **Runtime source of truth (live, consumed by all three runtimes):**
  `~/.local/share/iaa/` — README.md + `iaa/` skill
  directory (SKILL.md v3, sha256 `fee98091…`). Unchanged since 2026-08-27 16:22.
- **Maintained/versioned source of truth (this repository):** `iaa/`
...
(hash-verify). The repo's `iaa/` subtree must always hash-match the
...
| `iaa/scripts/manage.sh` | install/verify/uninstall | **canonical** |
...
`.zip`, the 10 İAA-named config backups, session-transcript dirs, the Codex diagnostics raw
