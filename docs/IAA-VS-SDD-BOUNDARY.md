# İAA vs SDD Boundary (current, tested)

The exact rules that separate Adaptive İAA from native Superpowers SDD, frozen as of
...
| Dimension | İAA (adaptive) | superpowers:subagent-driven-development 6.3.0 (cadence lines verified verbatim-unchanged at 6.4.1 — release-hardening/01) |
|---|---|---|
...
2. **İAA mode never loads SDD.** Not "overrides" — *never loads*. Selection-level exclusion,
...
5. **Component skills stay usable** (whitelist: TDD, worktrees, verification, receiving/
   ...
   lanes İAA already authorized.
6. **In native mode, İAA stands down entirely** — no second authority on top of SDD.
...
- Native opt-in works: 2 samples (archfix B: 15 agents; boundary C: 11 agents), İAA absent.
...
session; İAA installs no hook and relies on the user-instruction channel (CLAUDE.md/AGENTS.md),
